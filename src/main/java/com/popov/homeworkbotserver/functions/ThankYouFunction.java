 ...
 * @author Anton Popov
 */
public class ThankYouFunction implements IFunction {

    private final Pattern thankYouRegexp = Pattern.compile("(спасиб.*|спс)");

    @Override
    public boolean isMatch(String text) {
        return thankYouRegexp.matcher(text).matches();
    }
    ...
    public void execute(String inputText, List<Attachment> attachments, Object userId, DatabaseReference ref, IMessageSender messageSender, Map<String, String> locale) {
        messageSender.sendMessage(locale.get("thankYou"), userId);
    }
}
